class Category {
    ...
    addItems() {
        ...
        document.querySelector(`#tab-${this.id} > div.card__inner.\\[.js-expander.\\] > span > button`).addEventListener('click', sortList)
    }
        
    addUpdatedListItems(i) {
        const items = this.findItems()
        const ulLis = document.querySelector(`#tab-${i} div.card__expander ul`)
        items.forEach(item => {
            const li = document.createElement('li')
        li.className = 'item'
        li.id = `item-${item.id}`
    ...
    removeLisFromUl(listItemsInUl) {
        while (listItemsInUl().firstChild) 
        listItemsInUl().removeChild(listItemsInUl().lastChild)
    }
